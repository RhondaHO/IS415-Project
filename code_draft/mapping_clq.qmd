...
amenity_name <- names(bus_clq)[2]
amenity_pvalue <- names(bus_clq)[3]

tmap_mode("view")
tm_shape(mpsz_sf) +
  tm_polygons() +
tm_shape(bus_clq, subset = bus_clq$amenity_pvalue < 0.05) + #filter out p_value less than 0.05 
...
```{r}
plot_clq <- function(data, colA, p_sim_col) {
  
  # filter the bus data to keep only those with a p-value less than 0.05
  clq_filtered <- subset(data, data[, p_sim_col] < 0.05)
  
  # create the tmap plot
  tmap_mode("view")
  tm_shape(mpsz_sf) +
    tm_polygons() +
    tm_shape(clq_filtered) +
    tm_dots(col = clq_filtered[, colA],
            size = 0.01,
            border.col = "black",
            border.lwd = 0.5)
}
...
```{r}
plot_clq(bus_clq,"Bus","p_sim_Bus")
```
